...
<html>
	...
	<body vLink="#006666" link="#003366" bgColor="#E0F0F8">
		...
		<% int id = (Integer)session.getAttribute("role"); %>
			<p style="float:left;"><b>你正在做的事情是：人力资源-->人力资源档案管理-->人力资源档案查询</b></p>
		...
		<table bgcolor="" align="center" border="1" bordercolor="">
			...
			<s:iterator value="List" id="msg">
				<tr>
					...
					<td align="center">
						<s:a href="toPeopleInfo.action?people.id=%{#msg.id}">查看明细</s:a>
						<s:if test="(#msg.state)!=2">
						<s:a href="toPeopleInfo_toUpdate.action?people.id=%{#msg.id}">更改</s:a>
						</s:if>
					<% if(id==2){ %>
						<s:if test="(#msg.state)==0">
							<s:a href="toPeopleInfo_checkToUpdate.action?people.id=%{#msg.id}">复核</s:a>
						</s:if> 
						<s:if test="(#msg.state)==2">
							<s:a href="updatePeople_recover.action?people.id=%{#msg.id}">恢复</s:a>
						</s:if>
						<s:if test="(#msg.state)!=0">
							<s:a href="deletePeople.action?people.id=%{#msg.id}">删除</s:a>
						</s:if> 	
					<% } %>
					</td>
